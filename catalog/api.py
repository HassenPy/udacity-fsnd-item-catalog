    ...
    def post(self):
        ...
        if category.is_valid():
            try:
                ...
                message = jsonify({
                    "error": "resouce already exists."
                })
                return make_response(message, 409)
        return make_response(jsonify({'errors': category.errors}), 400)
    ...
    def post(self):
        ...
        if item.is_valid():
            ...
            try:
                ...
                return make_response(message, 409)
        return make_response(jsonify({'errors': item.errors}), 400)
...
catalogAPI.add_url_rule('/api/item/',
                        view_func=item_view, methods=['GET', ])
catalogAPI.add_url_rule('/api/item/<int:id>/',
                        view_func=item_view, methods=['GET', ])
catalogAPI.add_url_rule('/api/item/',
