...
class authTest(baseTest):
    ...
    def test_unauthenticated_login_page_render(self):
        """Test login page display for unauthenticated user."""
        response = self.app.get("/login")
        assert(response.status_code == 200)

    def test_authenticated_login_page_redirect(self):
        """Test if login page redirects authenticated user."""
        with self.app as c:
    ...
    def test_login_form_csrf_protection(self):
        """Test login form csrf protection test."""
    ...
    def test_login_form_error(self):
        """Test login form with erroneous data."""
        with self.app as c:
            token = c.get("/csrf-token")
            token = json.loads(token.data.decode())["_csrf_token"]
            response = c.post("/login", data={
    ...
    def test_unauthenticated_logout(self):
        """Test if logout is logout_required protected."""
        response = self.app.get("/logout")
        assert(response.status_code == 401)

    def test_authenticated_logout(self):
        """Test if logout view logs authenticated user out."""
        with self.app as c:
            token = c.get("/csrf-token")
            token = json.loads(token.data.decode())["_csrf_token"]
            response = c.post("/login", data={
    ...
    def test_unauthenticated_signup_page_render(self):
        """Test if signup page displays for unauthenticated user."""
        response = self.app.get("/signup")
        assert(response.status_code == 200)

    def test_authenticated_signup_page_redirect(self):
        """Test if signup page redirects authenticated user."""
        with self.app as c:
            token = c.get("/csrf-token")
            token = json.loads(token.data.decode())["_csrf_token"]
            c.post("/login", data={
    ...
    def test_signup_form_csrf_protection(self):
        """Signup form csrf protection test."""
